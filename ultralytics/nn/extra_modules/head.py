...
__all__ = ['Detect_DyHead', 'Detect_DyHeadWithDCNV3', 'Detect_DyHeadWithDCNV4', 'Detect_AFPN_P345', 'Detect_AFPN_P345_Custom', 'Detect_AFPN_P2345', 'Detect_AFPN_P2345_Custom', 'Detect_Efficient', 'DetectAux',
           'Segment_Efficient', 'Detect_SEAM', 'Detect_MultiSEAM', 'Detect_DyHead_Prune', 'Detect_LSCD', 'Segment_LSCD', 'Pose_LSCD', 'OBB_LSCD', 'UTAH', 'Segment_TADDH', 'Pose_TADDH', 'OBB_TADDH',
...
class TaskDecompositionLoRA(nn.Module):
    def __init__(self, feat_channels, stacked_convs, la_down_rate=8):
        super(TaskDecomposition, self).__init__()
        self.feat_channels = feat_channels
        self.stacked_convs = stacked_convs
        ...
        self.reduction_conv = ConvWithLoRA(self.in_channels, self.feat_channels, 1)
        self.init_weights()
...
class UTAH_LoRA(nn.Module):
    ...
    def __init__(self, nc=80, hidc=256, ch=()):
        ...
        self.share_conv = nn.Sequential(
            ConvWithLoRA(hidc, hidc // 2, 3),  # 第一个卷积 + LoRA
            ConvWithLoRA(hidc // 2, hidc // 2, 3)  # 第二个卷积 + LoRA
        )

        self.cls_decomp = TaskDecompositionLoRA(hidc // 2, 2, 16)
        self.reg_decomp = TaskDecompositionLoRA(hidc // 2, 2, 16)
        self.DyDCNV2 = DyDCNv2(hidc // 2, hidc // 2)
        self.spatial_conv_offset = ConvWithLoRA(hidc, 3 * 3 * 3, 3, rank=8)
        self.offset_dim = 2 * 3 * 3
        self.cls_prob_conv1 = nn.Conv2d(hidc, hidc // 4, 1)
        self.cls_prob_conv2 = nn.Conv2d(hidc // 4, 1, 3, padding=1)
        self.cv2 = nn.Conv2d(hidc // 2, 4 * self.reg_max, 1)
...
class LoRAAdapter(nn.Module):
    def __init__(self, in_dim, out_dim, rank=4):
        super().__init__()
        self.lora_A = nn.Parameter(torch.randn(in_dim, rank))  # 低秩矩阵A
        self.lora_B = nn.Parameter(torch.zeros(rank, out_dim))  # 低秩矩阵B
        self.scale = 1.0  # 可调整的缩放系数

    def forward(self, original_weight):
        # 返回原始权重 + 低秩调整量
        return original_weight + self.scale * (self.lora_A @ self.lora_B)
...
class ConvWithLoRA(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, rank=4):
        ...
        self.lora = LoRAAdapter(in_channels * kernel_size * kernel_size,
                                out_channels * kernel_size * kernel_size, rank)

    def forward(self, x):
        # 动态生成LoRA权重
        lora_weight = self.lora(self.conv.weight.view(self.conv.weight.size(0), -1))
        lora_weight = lora_weight.view(self.conv.weight.shape)

        return F.conv2d(x, lora_weight, self.conv.bias,
                        stride=self.conv.stride, padding=self.conv.padding)



class Segment_TADDH(UTAH):
    """YOLOv8 Segment head for segmentation models."""
